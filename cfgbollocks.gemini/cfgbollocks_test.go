...
`
	...
	if entries[1].Key != "1.2.3" {
		t.Errorf("Expected key '1.2.3', got %q", entries[1].Key)
	}
	if entries[1].Separator != ":" {
		t.Errorf("Expected separator ':', got %q", entries[1].Separator)
	}
}
...
FIN
`
	p1 := NewParser(strings.NewReader(input))
	entries1, err := p1.Parse()
	if err != nil {
		t.Fatalf("Initial parse failed: %v", err)
	}

	var buf strings.Builder
	if err := Write(&buf, entries1); err != nil {
	...
	if len(entries1) != len(entries2) {
		t.Fatalf("Count mismatch: %d vs %d", len(entries1), len(entries2))
	}
	...
	for i := range entries1 {
		...
		if entries1[i].Separator != entries2[i].Separator || entries1[i].Delimiter != entries2[i].Delimiter {
			t.Errorf("Metadata mismatch at %d", i)
		}
	}
}
...
func TestBootstrapStrictness(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:   " cfgbollocks ~ ###\nformat=v1\n###",
			wantErr: true,
			msg:     "file must begin with 'cfgbollocks'",
		},
		{
