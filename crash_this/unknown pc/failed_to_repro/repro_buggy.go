...
func main() {
	...
	fmt.Println("Will call the exact same pattern as your GetServiceNamesFromPIDUncached path ~100 000 times")
	fmt.Println("with forced GC after every 100 calls. Expect 0xc0000005 soon.")

	start := time.Now()
	for i := 0; i < 100_000; i++ {
		_, err := getServiceNamesRepro(uint32(i % 5000)) // fake PID cycling
		if err != nil && i%1000 == 0 {
			fmt.Printf("err at %d: %v\n", i, err)
		}
	...
	fmt.Printf("Survived 100k calls in %v — bug did NOT trigger this time (or was fixed)\n", time.Since(start))
}
...
func getServiceNamesRepro(targetPID uint32) ([]string, error) {
	...
	buffer, err := callWithRetryRepro("repro", 0, func(bufPtr *byte, sizePtr *uint32) error {
		...
		errEnum := windows.EnumServicesStatusEx(
			...
			bufPtr,
			*sizePtr,
			sizePtr, // ← this pointer comes from the stack in callWithRetryRepro
			&servicesReturned,
			&resume,
			nil,
		)
	...
	// minimal parse (not important for crash repro)
	return nil, nil
}

// This is EXACTLY your current callWithRetry (stack &size)
func callWithRetryRepro(who string, initialSize uint32, call func(bufPtr *byte, sizePtr *uint32) error) ([]byte, error) {
	...
	for tries := 0; tries < MAX_RETRIES; tries++ {
		...
		runtime.KeepAlive(&size) // does NOT help — the pointer was already escaped
