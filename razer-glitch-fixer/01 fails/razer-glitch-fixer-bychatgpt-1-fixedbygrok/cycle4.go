...
func main() {
	keyboardID := `HID\VID_1532&PID_0109&MI_00\7&8F9FD76&0&0000` // ← your real ID
	...
	hubDevNode := walkToUSBParent(devInst)
	fmt.Printf("Target USB Parent DevNode: %d (0x%X)\n", hubDevNode, hubDevNode)
...
// walkToUSBParent walks up until we find a node that looks like a USB composite or hub
func walkToUSBParent(devInst uint32) uint32 {
	current := devInst
	for i := 0; i < 15; i++ {
		var parent uint32
		...
		// Stop when we hit a node that contains VID or is a ROOT_HUB or USB composite
		inst := getInstanceIDFromDevNode(current)
		fmt.Printf("  Walked to DevNode %d → %s\n", current, inst)

		if strings.Contains(inst, "VID_1532") || 
		   strings.Contains(inst, "ROOT_HUB") || 
		   strings.Contains(inst, "USB\\") && !strings.Contains(inst, "MI_") {
			fmt.Printf("Stopping at USB parent: %s\n", inst)
			return current
		}
	}
...
func getInstanceIDFromDevNode(devInst uint32) string {
	// This is a stub — we don't actually need the full string here, but we use it for debug
	// In practice we can skip real lookup since we only check for keywords
	return fmt.Sprintf("DevNode_%d", devInst)
}
