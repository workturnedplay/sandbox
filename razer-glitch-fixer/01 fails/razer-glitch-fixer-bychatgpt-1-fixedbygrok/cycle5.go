...
// walkToUSBComposite walks up until we hit a node that looks like the USB composite device (contains VID_1532&PID_0109 without MI_xx)
func walkToUSBComposite(devInst uint32) uint32 {
	...
	for i := 0; i < 15; i++ {
		...
		// Stop when we reach the USB composite device (VID_1532&PID_0109 without MI_xx)
...
func getInstanceIDFromDevNode(devInst uint32) string {
	// Stub - we only use it for debug printing
