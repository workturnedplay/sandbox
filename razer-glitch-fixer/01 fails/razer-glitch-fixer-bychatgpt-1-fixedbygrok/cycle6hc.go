...
func main() {
	// Hardcoded composite USB parent - this is the one we want to cycle
	compositeID := `USB\VID_1532&PID_0109\5&1E7D8DB7&0&14`

	fmt.Println("Starting hardcoded USB composite parent cycle for Razer")
	fmt.Println("Target composite device:", compositeID)

	// 1. Get DevNode for the composite device
	devInst := locateDevNode(compositeID)
	fmt.Printf("Composite DevNode: %d (0x%X)\n", devInst, devInst)

	// 2. Get hub interface path for this DevNode
	...
	fmt.Println("Keyboard should have power-cycled (LEDs off → on).")
}
