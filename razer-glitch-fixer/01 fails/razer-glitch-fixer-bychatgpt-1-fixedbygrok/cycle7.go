...
func main() {
	keyboardID := `HID\VID_1532&PID_0109&MI_00\7&8F9FD76&0&0000` // change if needed
	...
	fmt.Printf("Target USB Composite / Hub DevNode: %d (0x%X)\n", hubDevNode, hubDevNode)
	...
	fmt.Println("Keyboard should be fully reset.")
}
...
func getInstanceIDFromDevNode(devInst uint32) string {
	// This function is only for debug - we try to get the InstanceId string
//	var buf [512]uint16
//	var needed uint32
	// We can't easily get InstanceId from DevNode with public APIs, so we return a placeholder
...
func walkToUSBComposite(devInst uint32) uint32 {
	...
	for i := 0; i < 15; i++ {
		...
		// Stop when we see a node that looks like the USB composite device
		if strings.Contains(inst, "VID_1532&PID_0109") && !strings.Contains(inst, "MI_") {
			fmt.Printf("Stopping at USB Composite Parent: %s\n", inst)
			return current
		}
	}
