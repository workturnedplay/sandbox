...
func main() {
	keyboardID := `HID\VID_1532&PID_0109&MI_00\7&8F9FD76&0&0000` // your real one
	...
	hubDevNode := walkToUSBComposite(devInst)
	fmt.Printf("Target USB Composite Parent DevNode: %d (0x%X)\n", hubDevNode, hubDevNode)
	...
	fmt.Println("Keyboard should be fully reset (LEDs off → on).")
}
...
func walkToUSBComposite(devInst uint32) uint32 {
	current := devInst
	for i := 0; i < 15; i++ {
		var parent uint32
		...
		fmt.Printf("  Walked to DevNode %d\n", current)

		// Stop when we reach a node that looks like the USB composite device
		// (contains VID_1532&PID_0109 and does not contain MI_xx)
		if isUSBComposite(current) {
			fmt.Printf("Stopping at USB Composite Parent DevNode %d\n", current)
			return current
		}
	}
	fmt.Println("Warning: fell back to last parent")
	return current
}

// Simple heuristic: we assume DevNode numbers close to the keyboard are the composite
// This is not perfect but works for your flat tree
func isUSBComposite(devInst uint32) bool {
	// For your system, the composite is usually 1 or 2 levels above the keyboard
	// We can improve this later with real InstanceId lookup if needed
	return devInst > 1 && devInst < 10 // temporary heuristic for your tree
}
