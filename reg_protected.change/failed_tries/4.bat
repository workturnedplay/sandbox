...
:: -------------------------------
:: Require Administrator (Filter Manager driver handle)
:: -------------------------------
fltmc >nul 2>&1
if not "%errorlevel%"=="0" (
...
:: -------------------------------
:: Execute embedded payload in-memory
:: -------------------------------
set "BATCH_PATH=%~f0"
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$s=[System.IO.File]::ReadAllText($env:BATCH_PATH); $start=$s.IndexOf('===PS_START===')+14; iex $s.Substring($start).TrimStart()"
exit /b %errorlevel%
...
function Set-RegistryDwordSafe {
    ...
    # Explicit edge-case handling: Create key hierarchy if missing
